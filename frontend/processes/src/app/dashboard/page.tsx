...
import { ArrowRight, ArrowLeft, Filter, Calendar, 
  Building, Box, FileText, ShoppingCart, TrendingUp, 
  Layers, Search, LucideIcon } from 'lucide-react';
...
const mockPurchaseOrders: PurchaseOrder[] = [
  { id: 'PO-2025-001', date: '28/02/2025', department: 'แผนกยา', items: 12, total: 45000, status: 'เสร็จสิ้น' },
  { id: 'PO-2025-002', date: '27/02/2025', department: 'แผนกเวชภัณฑ์', items: 8, total: 32000, status: 'รออนุมัติ' },
  { id: 'PO-2025-003', date: '26/02/2025', department: 'แผนกเครื่องมือแพทย์', items: 3, total: 120000, status: 'กำลังดำเนินการ' },
  { id: 'PO-2025-004', date: '24/02/2025', department: 'แผนกสำนักงาน', items: 15, total: 18000, status: 'เสร็จสิ้น' },
  { id: 'PO-2025-005', date: '20/02/2025', department: 'แผนกยา', items: 7, total: 28000, status: 'เสร็จสิ้น' },
];
...
export default function Dashboard() {
  ...
  const FilterButton: React.FC<FilterButtonProps> = ({ icon, label, onClick, isActive }) => {
    const Icon = icon;
    return (
      <button 
        onClick={onClick}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">รหัส</th>
                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">วันที่</th>
                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">หน่วยงาน</th>
                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">จำนวนรายการ</th>
                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">มูลค่ารวม</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {mockPurchaseOrders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{order.department}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{order.items}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">฿ {order.total.toLocaleString()}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm">
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                      ${order.status === 'เสร็จสิ้น' ? 'bg-green-100 text-green-800' : 
                        order.status === 'รออนุมัติ' ? 'bg-yellow-100 text-yellow-800' : 
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2">
              <span className="text-lg font-semibold text-blue-600">Maechan Hospital</span>
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        <div className="flex justify-between mb-6">
          <h1 className="text-2xl font-semibold text-gray-800">Dashboard Inventory</h1>
          <div className="flex space-x-2">
            <button 
              onClick={() => setCurrentPage(0)}
              className={`px-4 py-2 rounded-md text-sm flex items-center gap-2 transition-colors ${currentPage === 0 ? 'bg-blue-500 text-white' : 'bg-white text-gray-600 border border-gray-200'}`}
            >
              ภาพรวม
            </button>
            <button 
              onClick={() => setCurrentPage(1)}
              className={`px-4 py-2 rounded-md text-sm flex items-center gap-2 transition-colors ${currentPage === 1 ? 'bg-blue-500 text-white' : 'bg-white text-gray-600 border border-gray-200'}`}
            >
              เปรียบเทียบ PR/PO
            </button>
          </div>
